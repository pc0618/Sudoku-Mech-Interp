...
def modified_eval_step(state, batch, config):
    """Modified eval step that returns pre-logit features."""
    def _forward(params):
        ...
        return output, updated_vars['intermediates']['pre_logits']
    
    return _forward(state.params)

def save_sequence_features(features, predictions, file_num):
    """Save sequence of features and predictions."""
    np.savez(f'sequence_features_{file_num}.npz',
             features=features,
...
def evaluate_and_collect_features():
    ...
    eval_step = functools.partial(modified_eval_step, config=model_config)
    ...
    print(f"Starting puzzle processing...")
        ...
        for batch_start in range(0, total_puzzles, batch_size):
            try:
                ...
                if len(accumulated_features) >= puzzles_per_save or total_processed >= total_puzzles:
                    ...
                    save_sequence_features(accumulated_features_array, accumulated_predictions_array, file_count)
                    file_count += 1
                    accumulated_features = []
                    accumulated_predictions = []
